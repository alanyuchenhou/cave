...
knowledge of nodes) from known links' weights (relations between nodes) and 
...
We also present a brief node embedding analysis.
...
\begin{table}[!ht]\centering
	\caption{
		The link list representation of the example in Figure \ref{fig:example}.
	}
...
appearing in natural language processing (embeddings for word embeddings), recommender systems (embeddings for users and items),
...
All these techniques have achieved high prediction accuracies in their various applications including language modeling, document classification, item rating prediction, node classification.
...
\begin{align*}
	&context(mouse) = \{\\
	&monitor, keyboard, printer, scanner\\
	&\}
\end{align*}
...
The reason is these node vectors do not have any spacial property
...
\begin{itemize}
	\item Airport \cite{colizza2007reaction}. Nodes represent 500 busiest airports in the United States, and each of the 5960 directed edges is weighted by the number of passengers traveling from one airport to another.
	\item Collaboration \cite{pan2012world}. Nodes represent 226 nations on Earth, and each of the 20616 edges	is weighted by a normalized count of academic papers whose author lists include that pair of nations.
	\item Congress \cite{porter2005network}. Nodes represent the 163 committees in the 102nd United States Congress, and each of the 26569 edges is weighted by the pairwise normalized interlock value of shared members.
	\item Forum \cite{opsahl2009clustering}. Nodes represent 1899 users of a student social network at UC Irvine, and each of	the 20291 directed edges is weighted by the number of messages sent between users.
\end{itemize}
The statistics of these datasets are summarized in
Figure \ref{fig:statistics} and Table \ref{tab:datasets}.
\begin{figure}[!ht]\centering
	\begin{subfigure}{0.4\textwidth}
		\includegraphics[width=\textwidth]{number-of-nodes}
		\caption{Number of nodes}
		\label{fig:number-of-nodes}
	\end{subfigure}
	\begin{subfigure}{0.4\textwidth}
		\includegraphics[width=\textwidth]{number-of-edges}
		\caption{Number of links}
		\label{fig:number-of-edges}
	\end{subfigure}
	\begin{subfigure}{0.4\textwidth}
		\includegraphics[width=\textwidth]{average-degree}
...
\begin{table}[!ht]\centering
	...
	\begin{tabular}{cccc} \hline \rowcolor{blue!30}
		...
		Congress & 163  & 26569 & 36.54 \\ \hline
		Forum  & 1899 & 20291 & 14.57 \\ \hline
	\end{tabular}
	\label{tab:datasets}
\end{table}
...
\begin{figure*}[!ht]\centering
	\begin{lstlisting}
		...
		def evaluate_model_on(dataset):
			(train_set, validation_set, test_set) = split(dataset)
			while validation_error decreases:
				train_error = estimator.learn(train_set)
...
Even though Model R outperforms some of the latest link weight prediction techniques by a large margin, the knowledge it learns is not clear yet.
...
However, we need to provide evidence that the node embeddings are the meaning knowledge of nodes the model learns.
...
attaches the metadata to the embeddings and produces the final visualization.
...
\begin{itemize}
	...
	Nodes represent 226 nations on Earth, and each of the 20616 edges is weighted by a normalized count of academic papers whose author lists include that pair of nations.
	The previous experiment uses this dataset in prediction accuracy evaluation.
	\item MovieLens100K \cite{harper2015movielens}.
	This dataset is a recommendation dataset, and also a bipartite graph dataset.
	Nodes represent 1000 users and 1700 items, i.e., movies, and each of the 100000 edges is weighted by the rating score a user has given to a movie.
\end{itemize}
...
with an extra the step to log embedding layer weights.
...
\subsubsection{Similarity and distance measurement}
...
\subsubsection{Prediction accuracy measurement}
