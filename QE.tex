...
\begin{figure}[h]
	...
	% Annotate the layers
	\node[annot,above of=H-1, node distance=1cm] (hl) {embedding layer};
	\node[annot,above of=I-2] {input layer};
	\node[annot,above of=O-2] {output layer};
	\end{tikzpicture}
	
	\caption{The skip-gram model with vocabulary size 8 and embedding size 4: the model consists of only 3 layers: an input layer expecting one-hot activation, an embedding layer of linear units, and an output layer of softmax units. For example, given sequence = [$ w_3, w_2, w_8, w_1, w_4 $], we have word = [$ w_8 $](in the input layer, $ w_8 $ is 1 and others are 0) and context = [$ w_1, w_2, w_3, w_4 $](in the output layer, $ w_1, w_2, w_3, w_4 $ are 1 and others are 0). The weights between the input layer and the embedding layer are the word vectors shown in Table \ref{tab:word}. These word vectors are randomly initialized and learned by the neural net during training. The embedding layer's activation is equal to the word vector of the current word due to one-hot activation in the input layer.}
	\label{fig:skipGram}
\end{figure}
