...
\begin{figure}[h]
	...
	\node (relu2) [process, below of=linearRegression] {ReLU[d=16]};
	...
	\caption{The multi-task prediction model with movie set size m, user set size n, embedding size 64: the model consists of only 5 layers - an input layer with one-hot activation, an embedding layer of linear units, 2 hidden layers of rectified linear unites and an output layer of a linear regression unit and a logistic regression unit. The units in each layer are not shown with d referring to the layer's dimension (number of units in the layer).}
	\label{fig:multiTask}
\end{figure}
