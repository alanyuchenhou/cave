...
\begin{figure}[h]
	...
	\end{tikzpicture}
	\caption{The conceptual fully-connected neural net model for a dataset with 
		1700 movies and 1000 users with layer size 64:
...
\autoref{fig:actural} shows the actual implementation with 2 modifications:
\begin{itemize}
	\item The input layer becomes a 2 mapping tables: they map node IDs to node 
	vectors, just like the word to vector mapping table shown in 
	\autoref{tab:word}
	\item The mapping layer becomes a 2-channel input layer: it is directly 
	activated by the output of the mapping table while its activations are 
	still those 2 node vectors
\end{itemize}
\begin{figure}[h]
	\centering
	\begin{tikzpicture}[node distance=2cm]
