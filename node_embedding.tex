...
much the user like to talk to the other user.
...
\subsection{Datasets}
There are 2 recent publications where a large number of collaborative filtering 
variants are benchmarked \cite{hwang2016efficient} \cite{polatidis2016multi}. 
...
The estimator uses a fully-connected neural net model, shown in 
...
\autoref{fig:actural} shows the actual model with 2 modifications from the 
conceptual model:
\begin{itemize}
...
\end{lstlisting}
This early stopping technique can work well in case validation error 
...
\autoref{fig:trainnig} shows 2 example learning processes with error traces.
\begin{figure}[h]
	\centering
	...
	\end{subfigure}
	\caption{2 example learning processes on different datasets:
...
\autoref{tab:accuracy} shows that node mapping outperforms all variants of 
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{ |X|c|c| }  \hline
		...
		\textbf{Node mapping lowest MAE} \\ \hline
...
prediction for a specific parameter setting) takes around 1 minute on MovieLens 
100K dataset and 8 minutes on MovieLens 1M dataset.
