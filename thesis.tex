...
\begin{align*}
	...
	&= \frac{1}{1 + exp(Wx+b)}\\
\end{align*}
where the output is exactly the probability of expected output being 1.
...
\[y = RFFM(x) = cos(\Omega x + b)\]
where $ y \in R^D, x \in R^d, \Omega \in R^{D \times d}, b \in R^D $ and cosine is element wise \cite{rahimi2008random}.
In this example, the elements in $ \Omega $ and $ b $ are sampled from distributions such that the dot product (the similarity) of two data points in the new space approximates the radial basis function (RBF) kernel value of the two data points in the original space when D is large:
\begin{align*}
	y_1 \cdot y_2
